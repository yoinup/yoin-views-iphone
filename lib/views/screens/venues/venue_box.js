...
Yvi.VenueNearUserSquareImageView = Yn.SquareImageView.extend(Yn.Btap, {
  ...
  didInsertElement: function() {
    this._super();
    this.venueBox = this.nearestInstanceOf(Yvi.VenueBoxView);
  },

  bTap: function() {

    var product = this.venueBox.get('selected'),
        venue = this.venueBox.get('venue'),
...
Yvi.VenueBoxView = Em.View.extend({
  ...
  hasUsers: Em.computed(function() {

    return this.get('sixPeople').get('length') > 0;
