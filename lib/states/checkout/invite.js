Yvi.CheckoutInviteState = Em.State.extend(Yn.UpdateUser, Yvi.CheckoutClose, {
			...
			if ( App.isNative && !!product && !product.get('isFree') ) {
				...
				var self = this,
						properties = {amount: product.get('price'),
													currencyCode: product.get('priceCurrency'), 
  ...
  // create or resend invitation
    ...
    invitation.one('did'+action+'Error', function(error) {
      ...
      var errorMessage, parsedError;
      try {
        parsedError = JSON.parse(JSON.parse(error.responseText));
        errorMessage = parsedError._info_;
        //console.log(errorMessage);
        //errorMessage = error.responseText._info_;
      } catch(e) {}

      if (error.status === 404 || !!errorMessage) {

        var text = (!!errorMessage) ? errorMessage :I18n.t('message_not_found_invitation_error'),
            title = I18n.t('popup_title');        
